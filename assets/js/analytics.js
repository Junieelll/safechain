...
class AnalyticsEngine {
  ...
  applyFilters(filters) {
    this.filteredIncidents = this.allIncidents.filter((incident) => {
      ...
      const typeMatch =
        filters.incidentTypes.includes("all") ||
  ...
  getStatistics() {
    ...
    const resolutionRate =
      incidents.length > 0 ? (resolvedCount / incidents.length) * 100 : 0;
    ...
    return {
      ...
      respondingIncidents: incidents.filter((i) => i.status === "Responding")
        .length,
...
function updateCharts() {
  ...
  if (chartInstances.responseTime) {
    chartInstances.responseTime.data.datasets[0].data =
      ...
      <div class="space-y-6 max-h-[500px] overflow-y-auto pr-2">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-2 mb-3">
            <button onclick="setExportTimeline('last7days')" class="export-timeline-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
              exportConfig.timeline === "last7days"
                ? "bg-emerald-500 text-white border-emerald-500"
                : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
            }" data-timeline="last7days">
              Last 7 Days
            </button>
            <button onclick="setExportTimeline('last30days')" class="export-timeline-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
              exportConfig.timeline === "last30days"
                ? "bg-emerald-500 text-white border-emerald-500"
                : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
            }" data-timeline="last30days">
              Last 30 Days
            </button>
            <button onclick="setExportTimeline('last90days')" class="export-timeline-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
              exportConfig.timeline === "last90days"
                ? "bg-emerald-500 text-white border-emerald-500"
                : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
            }" data-timeline="last90days">
              Last 90 Days
            </button>
            <button onclick="setExportTimeline('custom')" class="export-timeline-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
          ...
          <!-- CUSTOM DATE RANGE -->
          <div id="customDateRange" class="grid grid-cols-2 gap-3 ${
            exportConfig.timeline === "custom" ? "" : "hidden"
          }">
            <div>
              <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1">From Date</label>
              <input type="date" id="exportFromDate" value="${
                exportConfig.fromDate
              }" class="w-full px-3 py-2 text-sm border-2 border-gray-300 dark:border-neutral-600 rounded-lg bg-gray-50 dark:bg-neutral-700 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" onchange="updateExportDateRange()">
            </div>
            <div>
              <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1">To Date</label>
              <input type="date" id="exportToDate" value="${
                exportConfig.toDate
              }" class="w-full px-3 py-2 text-sm border-2 border-gray-300 dark:border-neutral-600 rounded-lg bg-gray-50 dark:bg-neutral-700 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" onchange="updateExportDateRange()">
            </div>
          </div>
        ...
        <div>
          ...
          <div class="space-y-2">
            ...
            <!-- Incident Details -->
            <label class="flex items-center p-3 border-2 border-gray-200 dark:border-neutral-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-all">
              <input type="checkbox" id="export-incidents" checked onchange="toggleExportData('incidents')" class="w-5 h-5 appearance-none border-2 border-gray-300 rounded-md checked:bg-[#01AF78] checked:border-[#01AF78] focus:ring-2 focus:ring-emerald-100 focus:ring-offset-0 transition-all cursor-pointer bg-[length:10px_10px] bg-center bg-no-repeat checked:bg-[url('data:image/svg+xml;base64,')]">
              <div class="ml-3 flex-1">
            ...
            <!-- Chart Data -->
            <label class="flex items-center p-3 border-2 border-gray-200 dark:border-neutral-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-all">
              <input type="checkbox" id="export-charts" onchange="toggleExportData('charts')" class="w-5 h-5 appearance-none border-2 border-gray-300 rounded-md checked:bg-[#01AF78] checked:border-[#01AF78] focus:ring-2 focus:ring-emerald-100 focus:ring-offset-0 transition-all cursor-pointer bg-[length:10px_10px] bg-center bg-no-repeat checked:bg-[url('data:image/svg+xml;base64,')]">
              <div class="ml-3 flex-1">
            ...
            <label class="flex items-center p-3 border-2 border-gray-200 dark:border-neutral-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-700/50 transition-all">
              <input type="checkbox" id="export-residents" onchange="toggleExportData('residents')" class="w-5 h-5 appearance-none border-2 border-gray-300 rounded-md checked:bg-[#01AF78] checked:border-[#01AF78] focus:ring-2 focus:ring-emerald-100 focus:ring-offset-0 transition-all cursor-pointer bg-[length:10px_10px] bg-center bg-no-repeat checked:bg-[url('data:image/svg+xml;base64,')]">
              <div class="ml-3 flex-1">
...
window.setExportTimeline = function (timeline) {
  ...
  document.querySelectorAll(".export-timeline-btn").forEach((btn) => {
    ...
    if (btnTimeline === timeline) {
      btn.className =
        "export-timeline-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-emerald-500 text-white border-emerald-500";
    } else {
      btn.className =
        "export-timeline-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500";
    }
  });
...
function updateExportSummary() {
  ...
  summaryElement.innerHTML = `
    <strong>${includedCount} data section${
    includedCount !== 1 ? "s" : ""
  }</strong> selected for export <br>
...
function generateResidentInfo(incidents) {
  ...
  incidents.forEach((inc) => {
    ...
    resident.incidentTypes[inc.type] =
      (resident.incidentTypes[inc.type] || 0) + 1;
...
function downloadCSV(data) {
  ...
  if (data.incidents) {
    ...
    csv +=
      "ID,Type,Resident,Location,Time Reported,Response Time,Status,Priority,Assigned To,Device ID,Description\n";
  ...
  if (data.charts) {
    ...
    csv += "Type,Count\n";
    Object.entries(data.charts.incidentTypeDistribution).forEach(
      ([type, count]) => {
        csv += `${type},${count}\n`;
      }
    );
    ...
    csv += "Range,Count\n";
    Object.entries(data.charts.responseTimeDistribution).forEach(
      ([range, count]) => {
        csv += `${range},${count}\n`;
      }
    );
  ...
  if (data.deviceLogs) {
    ...
    csv +=
      "Device ID,Incident ID,Timestamp,Trigger Type,Sensor Status,Battery,Signal\n";
...
function downloadPDF(data) {
  ...
  content +=
    "Note: This is a simplified PDF. For full PDF functionality with charts, integrate jsPDF library.\n";
...
function downloadFile(content, mimeType, extension) {
  ...
  a.href = url;
  a.download = `SafeChain_Analytics_${
    ...
    <div class="space-y-6">
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-2 mb-2">
          <button onclick="setTimePeriod('last7days')" class="time-period-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
            filterState.timePeriod === "last7days"
              ? "bg-emerald-500 text-white border-emerald-500"
              : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
          }" data-period="last7days">
            Last 7 Days
          </button>
          <button onclick="setTimePeriod('last30days')" class="time-period-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
            filterState.timePeriod === "last30days"
              ? "bg-emerald-500 text-white border-emerald-500"
              : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
          }" data-period="last30days">
            Last 30 Days
          </button>
          <button onclick="setTimePeriod('last90days')" class="time-period-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
            filterState.timePeriod === "last90days"
              ? "bg-emerald-500 text-white border-emerald-500"
              : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
          }" data-period="last90days">
            Last 90 Days
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          }" class="w-full px-4 py-2.5 text-sm border-2 border-gray-300 dark:border-neutral-600 rounded-lg bg-gray-50 dark:bg-neutral-700 text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
        </div>
        <div>
          <label class="block text-xs font-bold text-gray-600 dark:text-gray-400 uppercase mb-2">
            To Date
...
function generateIncidentTypeButtons() {
  ...
  return types
    .map(
      (type) => `
    <button onclick="toggleIncidentType('${type}')" class="incident-type-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all ${
        filterState.incidentTypes.includes(type)
          ? "bg-emerald-500 text-white border-emerald-500"
          : "bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500"
      }" data-type="${type}">
...
function updateFilterButtons() {
  ...
  document.querySelectorAll(".time-period-btn").forEach((btn) => {
    ...
    if (period === filterState.timePeriod) {
      btn.className =
        "time-period-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-emerald-500 text-white border-emerald-500";
    } else {
      btn.className =
        "time-period-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500";
    }
  });
  ...
  document.querySelectorAll(".incident-type-btn").forEach((btn) => {
    ...
    if (filterState.incidentTypes.includes(type)) {
      btn.className =
        "incident-type-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-emerald-500 text-white border-emerald-500";
    } else {
      btn.className =
        "incident-type-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500";
    }
  });
  ...
  document.querySelectorAll(".status-btn").forEach((btn) => {
    ...
    if (filterState.status.includes(status)) {
      btn.className =
        "status-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-emerald-500 text-white border-emerald-500";
    } else {
      btn.className =
        "status-btn px-4 py-2.5 text-xs font-medium rounded-lg border-2 transition-all bg-white dark:bg-neutral-700 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-neutral-600 hover:border-emerald-500";
    }
  });
}
